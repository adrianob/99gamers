class ProjectPolicy < ApplicationPolicy
  ...
  def permitted_attributes
    if user.present? && (user.admin? || (record.draft? || record.rejected? || record.in_analysis?))
      ...
      p_attr << plan_attributes
      p_attr << account_attributes

      p_attr.flatten
    else
      [:funding_type, :transfer, :about_html, :video_url, :uploaded_image, :headline, :budget,
                 user_attributes, posts_attributes, budget_attributes, reward_attributes, goal_attributes, plan_attributes, account_attributes]
    end
  end
  ...
  def reward_attributes
    { rewards_attributes: [:_destroy, :id, :maximum_contributions,
                          :description, :title, :uploaded_image, :deliver_at, :minimum_value, benefits_attributes: [:description, :_destroy, :id]] }
  end

  def goal_attributes
    { goals_attributes: [:_destroy, :id, :title,
                          :description, :value] }
  end
