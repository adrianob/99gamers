class RemoveOnlineFromRecommended < ActiveRecord::Migration
  def change
    execute <<-SQL
    ...
    WITH recommended_projects AS (
         ...
         SELECT 'recurrent'::text AS origin,
          ...
          WHERE recurrents.funding_type_id = (( SELECT ft.id
                   FROM funding_types ft
                  WHERE ft.name = 'recurrent'::text)) AND recurrents.state::text = 'online'::text
          ORDER BY (random())
         LIMIT 3
        ), recents_projects AS (
         SELECT 'recents'::text AS origin,
          ...
          WHERE recents.state::text = 'online'::text AND (now() - recents.online_date) <= '2 mons'::interval AND NOT (recents.id IN ( SELECT recommends.id
